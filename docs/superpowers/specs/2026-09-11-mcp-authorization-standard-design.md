...
엔드포인트 명세 작성 원칙:
- **명세 문서가 기준이다.** 구현에서 역으로 옮겨 적지 않는다. 엔드포인트마다 근거 RFC·MCP
  조항에 정의된 파라미터·헤더·응답 필드를 **REQUIRED / RECOMMENDED / OPTIONAL 표시와 함께
  전부** 싣는다. 우리 구현이 쓰지 않는 OPTIONAL 항목도 빠뜨리지 않는다.
- **관측값은 대조용이다.** 종단 검증에서 캡처한 실제 요청·응답을 명세 옆에 붙인다.
- **차이는 드러낸다.** 명세와 관측이 다르면(누락된 REQUIRED, 다른 값 등) 구현을 고친다.
  고치지 않는 경우 문서에 그 차이와 이유를 적는다.
