...
## 프로젝트 스킬 `writing-practice-docs`

아래 "작성 규칙"은 이 저장소의 다른 practice 문서에도 계속 쓰므로 프로젝트 전용 스킬로 둔다.
위치는 `.claude/skills/writing-practice-docs/` 이고 저장소에 커밋한다. 전역(`~/.claude/skills/`)에는 두지 않는다.

| 파일 | 내용 |
|---|---|
| `SKILL.md` | 문서 3종 구성, 계층형 작성, 용어, 쓰지 않는 것, 명세 인용, 중복 처리 규칙 |
| `templates.md` | 허브·API 명세·시퀀스·README 틀 |
| `terms.txt` | 한국어로 옮기면 안 되는 용어 목록(정규식과 영어 표기) |
| `scripts/check_docs.py` | 기계 검사(범용, 파일 목록을 인자로 받음). `--links-from` 으로 옛 문서의 링크가 모두 옮겨졌는지 확인 |
| `scripts/test_check_docs.py` | 검사 스크립트 테스트 |

- 스킬은 superpowers writing-skills 절차대로 만든다. 스킬 없이 쓴 문서에서 위반을 먼저 확인하고(RED), 스킬을 따라 쓴 문서에서 위반이 사라지는지 확인한다(GREEN).
- 이번 12개 파일 작업은 이 스킬을 따른다. 12개 파일의 목차, 대상 엔드포인트와 시퀀스 목록, 옮겨야 하는 내용은 이 설계에 남는다.
...
- 순서: 스킬 → 최상위 3종(API 명세 → 시퀀스 → 허브) → official 3종 → chat-memory 3종 → community 3종.
...
- **기계 검사** (12개 파일, 스킬의 `scripts/check_docs.py`)
  - 내부 링크와 앵커가 살아 있는가
  - mermaid 블록 문법이 맞는가
...
- **누락**: "옮겨야 하는 내용"이 새 문서 어딘가에 모두 있는가. 옛 `MCP-AUTHORIZATION.md` 의 링크는 `--links-from` 으로 확인하고, 일부러 뺀 링크는 이유와 함께 목록 파일에 남긴다.
