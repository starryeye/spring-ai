...
- **API 키 미설정**: 설계 의도는 기동 시점 실패였으나, **실제 동작은 그렇지 않다.**
  openai 프로파일이 활성이고 `${OPENAI_API_KEY}` 를 어디서도 해석할 수 없는 상태에서도
  애플리케이션은 정상 기동한다(실측). 키 누락은 첫 채팅 요청 시점에 인증 오류로 드러난다.
  README 트러블슈팅에 이 사실을 명시했다.
  키 공급 경로는 두 가지다 — `product-agent/secrets.yml`(git 무시, `spring.config.import`
  으로 optional 로딩) 또는 환경변수. 환경변수가 파일보다 우선한다.
  이 사슬은 `SecretsFileImportTest` 가 회귀 테스트로 지킨다.
