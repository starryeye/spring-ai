...
- 테스트: 단계별 단위 테스트(PKCE 값, callback의 `state`·`iss` 검증, token 요청 모양, discovery 확인 규칙). 전체 흐름은 auth-server·shop-mcp-server를 띄워 한 번 돌리고 그 출력을 캡처로 남긴다(browser login은 사람이 한다. 자동화하지 않는다).
