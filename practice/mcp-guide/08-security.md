...
공격자가 서는 자리는 네 곳이다.
사용자가 client에 넣은 악성 MCP Server, 사용자가 연 악성 web page, 사용자 기기에 설치된 다른 프로그램, 그리고 네트워크 경로다.
절마다 공격, 막는 것, official에서 막는 곳을 차례로 쓴다.
...
| 8.5 | mix-up | 악성 MCP Server와 그 Authorization Server | callback의 `iss` 확인 |
| 8.6 | confused deputy (OAuth proxy) | 악성 client | client마다 받는 consent. official의 MCP Server는 proxy가 아니다 |
| 8.6 | 다른 MCP Server용 token | 악성 MCP Server | `resource`와 `aud` |
| 8.7 | token passthrough | token을 훔친 사람 | 자기용 token만 받기 |
| 8.8 | DNS rebinding, 같은 네트워크, 도청 | 악성 web page, 네트워크 경로 | `Origin`·`Host`, `127.0.0.1` bind, HTTPS |
| 8.9 | session hijacking | session ID를 알아낸 사람 | 요청마다 token 검사 |
...
`169.254.169.254`는 cloud 서버 안에서만 닿는 instance metadata 서비스 주소이고, 그 서버의 credentials를 돌려줄 수 있다.
cloud에서 도는 agent가 이 주소로 요청하고 응답이 오류 메시지 등으로 새어 나가면, 공격자는 밖에서 닿지 않던 credentials를 얻는다.
...
사용자 기기의 client에는 다른 위험이 있다.
`authorization_endpoint`가 `javascript:` 주소이면, client가 그 주소를 여는 순간 공격자의 script가 돈다.
주소를 shell 명령으로 열면, 주소 안의 글자가 명령으로 실행될 수도 있다.

**막는 것**

agent는 PRM이 알려 준 issuer가 credentials를 발급한 issuer와 같은지 먼저 보고, 다르면 metadata도 요청하지 않는다([3장](03-discovery.md)).
그래서 metadata 주소는 설정에 적은 issuer에서만 만들어진다.
metadata의 두 endpoint는 `https`이거나 loopback 주소의 `http`여야 해서, `javascript:`·`file:` 같은 주소는 여기서 멈춘다.

**official에서**

`McpAuthorizationDiscovery#discover`는 issuer를 비교한 뒤에야 metadata를 요청하고, `requireHttpUrl`로 endpoint 주소를 확인한다.
`local-client`의 `Discovery`도 같은 순서로 확인한다.
`local-client`는 browser를 shell 명령이 아니라 `java.awt.Desktop`의 `browse`로 연다([7장](07-local-client.md)).
`resource_metadata`와 PRM 주소는 확인 없이 요청한다(8.10).

## 8.3 Redirect URI 바꿔치기와 authorization code 가로채기

**공격**

공격자는 `client_id=official-shop-agent`와 자기 주소 `redirect_uri=http://evil.example/callback`을 넣은 authorization request 링크를 사용자에게 보낸다.
Authorization Server가 이 주소를 믿으면, 사용자가 login한 뒤 code는 공격자에게 간다.

redirect URI가 맞아도 code가 샐 수 있다.
loopback callback으로 오는 code는 같은 기기의 다른 프로그램이 가로챌 수 있고, redirect 주소는 browser 기록과 로그에도 남는다.
공격자는 손에 넣은 code를 token endpoint에 보내 token을 받으려 한다.

**막는 것**

Authorization Server는 `redirect_uri`를 등록한 값과 글자 그대로 비교하고, 다르면 그 주소로 redirect하지 않는다([4장](04-client-registration.md)).
loopback IP 주소만은 실행할 때마다 빈 포트를 쓰는 앱을 위해 포트를 빼고 비교한다.
loopback 주소로 가는 code는 이 기기를 떠나지 않으므로, 포트를 자유롭게 해도 code가 밖으로 새지 않는다.

새어 나간 code는 PKCE가 막는다([5장](05-authorization-and-token.md)).
token request에는 요청을 시작한 client만 가진 `code_verifier`가 있어야 한다.
`S256`의 `code_challenge`는 해시라서, authorization request 주소를 본 사람도 `code_verifier`를 알아내지 못한다.
...
public client의 `client_id`는 배포된 앱 안에 있어서 비밀이 아니다.
사용자 기기의 악성 프로그램이 `client_id=local-mcp-client`와 자기가 연 loopback 포트로 authorization request를 만들어 browser를 연다.
요청을 시작한 쪽이 이 프로그램이라 `code_verifier`도 이 프로그램에게 있어서, PKCE는 도움이 되지 않는다.
browser에는 사용자의 login session이 남아 있고, 사용자는 예전에 `local-mcp-client`에 consent한 적이 있다.
Authorization Server가 그 consent를 기억하면, browser는 화면 하나 없이 callback으로 넘어간다.
프로그램은 사용자 모르게 MCP Server용 access token을 받는다.

**막는 것**

public client의 요청에는 매번 consent를 받는다([4장](04-client-registration.md), [5장](05-authorization-and-token.md)).
사용자는 자기가 시작하지 않은 consent 화면을 보고 거절할 수 있다.
confidential client는 `client_secret`으로 자기를 증명하므로, 다른 프로그램이 그 `client_id`를 대도 token을 받지 못한다.

**official에서**

`local-mcp-client`는 `require-authorization-consent: true`로 등록되어 있다.
Spring이 consent를 건너뛰는 두 길은 클래스 하나씩이 막는다.
`PublicClientConsentService`는 public client의 consent를 저장하지 않는다.
`PublicClientScopeValidator`는 scope가 `openid` 하나뿐이거나 없는 요청을 `invalid_scope`로 거절한다.
...
MCP client는 PRM이 가리키는 Authorization Server를 따라가므로, 만나는 Authorization Server 가운데 하나가 공격자의 것일 수 있다.
...
1. client는 E와 흐름을 시작하고, E의 issuer와 `state`, `code_verifier`를 요청 기록에 남긴다 (1)(2).
2. E는 `resource`만 정상 MCP Server로 고치고, 나머지 parameter는 client가 보낸 그대로 둔 채 browser를 H로 보낸다 (3).
3. 사용자는 진짜 H의 화면에서 늘 쓰던 client에 consent하고, H는 code를 client의 callback으로 보낸다 (4)~(6).
4. client는 이 응답을 E와 시작한 흐름의 응답으로 알고, code와 `code_verifier`를 E의 token endpoint로 보낸다 (7).
5. 공격자는 받은 code와 `code_verifier`로 H에서 정상 MCP Server용 token을 받는다 (8)(9).

`state`는 client가 만든 값이 그대로 돌아오므로 맞는다.
PKCE도 막지 못한다. client가 `code_verifier`를 E에게 직접 보내기 때문이다.
...
official의 두 client는 한 Authorization Server에만 등록되어 있어서, PRM이 다른 곳을 가리키면 discovery에서 먼저 멈춘다(4장).
이 issuer 묶기는 Authorization Server를 하나만 쓰는 client의 방어이고, `iss` 확인은 여러 Authorization Server를 쓰는 client에서도 통한다.
...
**명세의 confused deputy: OAuth proxy인 MCP Server**

이 공격은 MCP Server가 제3자 API 앞에 선 OAuth proxy일 때 생긴다.
proxy는 제3자 Authorization Server에 고정된 `client_id` 하나로 등록되어 있고, MCP client에게는 DCR로 각자의 `client_id`를 준다.
...
제3자 Authorization Server는 cookie를 보고 consent 화면을 건너뛰고, proxy는 이 결과로 만든 MCP authorization code를 공격자의 redirect URI로 보낸다.
공격자는 그 code를 token으로 바꿔, 사용자로서 MCP Server와 그 뒤의 제3자 API를 쓴다.

막는 것은 proxy가 MCP client마다 받는 consent다.
proxy는 제3자 Authorization Server로 보내기 전에 자기 consent 화면을 보여 준다.
official에는 이 공격의 조건이 없다.
`shop-mcp-server`는 제3자 API를 부르지 않고 authorization endpoint도 없으며, `auth-server`는 DCR을 켜지 않는다.

**다른 MCP Server용 token**
...
official에서는 agent의 `ResourceIndicators`가 authorization·token·refresh request에 `resource`를 넣는다.
`auth-server`의 `ResourceIndicatorValidator`는 모르는 `resource`를 `invalid_target`으로 거절하고, `ResourceAudienceTokenCustomizer`는 `resource`를 `aud`에 넣는다.
MCP Server는 `audiences: http://localhost:8111/mcp` 설정으로 `aud`를 보고, `aud`가 `official-shop-agent`인 ID token에는 `401`로 답한다.
...
문서 API의 로그에는 이 요청이 사용자 본인의 요청처럼 남아, 누가 불렀는지 가리기 어렵다.
...
서버가 모든 network interface(`0.0.0.0`)에서 연결을 받으면, 카페 Wi-Fi 같은 네트워크의 다른 기기도 그 서버에 바로 연결한다.
...
이 검사는 token보다 먼저 하므로, token을 요구하지 않는 서버에서도 통한다.
...
`McpTransportSecurityFilter`가 SDK의 `DefaultServerTransportSecurityValidator`로 `Origin`과 `Host`를 보고, `McpTransportConfig`가 이 filter를 Spring Security 앞에 둔다.
세 앱은 `application.yml`의 `server.address: 127.0.0.1`로 이 기기 안의 연결만 받는다.

official은 HTTPS를 쓰지 않고 모두 `http://localhost`로 돈다.
학습용 환경이라 오가는 요청과 응답을 그대로 보려는 선택이다.
세 앱과 `local-client` 사이의 요청은 모두 loopback 주소 안에서 오가므로, 기기 밖의 네트워크를 지나지 않는다.
실제로 배포할 때는 HTTPS가 있어야 한다(8.10).
...
MCP Server는 session ID가 있어도 요청마다 token을 검사하고, session ID는 추측할 수 없는 값으로 만든다.
session을 연 사용자에 session을 묶어 두면, 자기 token을 가진 다른 사용자도 남의 session을 쓰지 못한다([6장](06-mcp-call-and-validation.md)).

**official에서**

`shop-mcp-server`의 `SecurityConfig`는 `anyRequest().authenticated()`로 모든 요청에 token을 요구하고, SDK는 session ID를 `UUID.randomUUID()`로 만든다.
official은 session을 사용자에 묶지 않는다.
agent가 MCP client 하나를 모든 사용자와 함께 쓰기 때문이고, 묶는 방법은 chat-memory practice에 있다(6장).
...
| SSE event `id` | 응답 event가 session ID를 `id`로 써서, 한 session에 같은 `id`의 event가 여럿 생긴다([1장](01-mcp-basics.md)). 끊긴 stream을 어디부터 이어 받을지 이 값으로 가리킬 수 없다 | 값은 Spring AI의 `WebMvcStreamableServerTransportProvider`가 정한다. 이 클래스가 `final`이라 transport 전체를 fork하지 않고는 바꿀 수 없다 |
| 서버에서 도는 agent의 SSRF 대응 | issuer를 먼저 비교하고 endpoint 주소를 확인한다. `resource_metadata`와 PRM 주소는 확인 없이 요청하고, 사설 IP 차단·redirect 대상 확인·egress proxy는 없다 | 모든 구성 요소가 한 기기의 `localhost`에서 도는 학습 환경이다 |

항목별 전체 판정은 [준수표](reference-compliance.md)에 있다.

## 8.11 다루지 않는 것

- Security Best Practices의 Local MCP Server Compromise와 stdio proxy 공격: 사용자 기기에서 client가 명령으로 띄우는 server의 문제다. 이런 server는 주로 stdio로 이야기하고, stdio에는 OAuth를 쓰지 않는다([1장](01-mcp-basics.md)).
- Scope Minimization: official은 인증된 요청에 모든 tool을 허용한다. scope 설계와 step-up authorization은 별도 practice의 주제다.
- CIMD 문서를 가져올 때의 SSRF와 `localhost` 사칭: official은 CIMD를 쓰지 않고, 규칙은 [4장](04-client-registration.md)에 있다.
...
```bash
# agent가 authorization request를 만들고, state를 session에 기록한다
...
# 다른 Authorization Server에서 온 것처럼 iss를 넣은 callback → 401, code는 token endpoint로 가지 않는다
...
다른 공격은 앞 장의 명령으로 재현한다: `Origin`·`Host`는 6장, 모르는 `resource`는 5장, credentials를 발급하지 않은 issuer는 7장.
ID token의 `aud`는 `docs/superpowers/captures/mcp-authorization-walkthrough.sh` 출력의 12단계, 등록하지 않은 `redirect_uri`는 `docs/superpowers/captures/mcp-authorization-supplement.sh` 출력의 4단계에 있다.

## 8.13 정리

- 공격자는 악성 MCP Server, 악성 web page, 같은 기기의 프로그램, 네트워크 경로에 선다.
- discovery는 issuer를 먼저 비교하고 endpoint 주소를 확인한다. authorization code는 redirect URI 정확 비교, PKCE `S256`, public client의 매번 받는 consent, callback의 `iss` 확인이 지킨다.
- token은 `resource`와 `aud`로 한 MCP Server에 묶이고, MCP Server는 자기용 token만 받아 downstream으로 넘기지 않는다. MCP 요청은 `Origin`·`Host`, `127.0.0.1` bind, 요청마다 하는 token 검사가 지킨다.
...
| 내용 | 명세 | 요구 수준 |
|---|---|---|
| 서버에 배포된 MCP client는 OAuth 관련 주소의 SSRF 위험에 대응한다(HTTPS, 사설 IP 차단, redirect 대상 확인, egress proxy). authorization 주소는 `https`와 개발용 loopback의 `http`만 받고, shell 명령으로 열지 않는다 | [MCP Security Best Practices — SSRF](https://modelcontextprotocol.io/specification/2025-11-25/basic/security_best_practices#server-side-request-forgery-ssrf), [OAuth Authorization URL Validation](https://modelcontextprotocol.io/specification/2025-11-25/basic/security_best_practices#oauth-authorization-url-validation), [RFC 9728 §7.7](https://www.rfc-editor.org/rfc/rfc9728#section-7.7) | MUST, MUST NOT, SHOULD |
...
| 여러 Authorization Server를 다루는 client는 mix-up을 막는다. code를 보내기 전에 `iss`를 기록한 issuer와 비교한다 | [MCP 2026-07-28 Security — Mix-Up Attacks](https://modelcontextprotocol.io/specification/2026-07-28/basic/authorization/security-considerations#mix-up-attacks), [Authorization Response Validation](https://modelcontextprotocol.io/specification/2026-07-28/basic/authorization#authorization-response-validation), [RFC 9207 §2.4](https://www.rfc-editor.org/rfc/rfc9207#section-2.4) | MUST |
| 고정 `client_id`를 쓰는 MCP proxy server는 제3자 Authorization Server로 보내기 전에, 동적으로 등록된 client마다 consent를 받는다 | [MCP Security Best Practices — Confused Deputy Problem](https://modelcontextprotocol.io/specification/2025-11-25/basic/security_best_practices#confused-deputy-problem), [MCP 2025-11-25 Authorization — Confused Deputy Problem](https://modelcontextprotocol.io/specification/2025-11-25/basic/authorization#confused-deputy-problem) | MUST |
| client는 `resource`를 넣고, MCP Server는 자기를 audience로 발급된 token만 받는다. MCP Server는 다른 token을 받거나 넘기지 않고, client는 그 MCP Server의 Authorization Server가 발급한 token만 보낸다 | [MCP 2025-11-25 Authorization — Access Token Privilege Restriction](https://modelcontextprotocol.io/specification/2025-11-25/basic/authorization#access-token-privilege-restriction), [Token Handling](https://modelcontextprotocol.io/specification/2025-11-25/basic/authorization#token-handling), [MCP Security Best Practices — Token Passthrough](https://modelcontextprotocol.io/specification/2025-11-25/basic/security_best_practices#token-passthrough) | MUST, MUST NOT |
| server는 모든 연결의 `Origin`을 검증하고, 로컬 server는 `127.0.0.1`에만 bind한다. Authorization Server의 endpoint는 HTTPS이고, redirect URI는 `localhost`이거나 HTTPS다 | [MCP 2025-11-25 Transports — Security Warning](https://modelcontextprotocol.io/specification/2025-11-25/basic/transports#security-warning), [MCP 2025-11-25 Authorization — Communication Security](https://modelcontextprotocol.io/specification/2025-11-25/basic/authorization#communication-security) | MUST, SHOULD |
| authorization을 구현한 MCP Server는 모든 요청을 검증하고 session을 인증에 쓰지 않는다. session ID는 추측할 수 없는 값으로 만들고 사용자 정보에 묶는다 | [MCP Security Best Practices — Session Hijacking](https://modelcontextprotocol.io/specification/2025-11-25/basic/security_best_practices#session-hijacking) | MUST, MUST NOT, SHOULD |
