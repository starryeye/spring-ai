...
4장에서는 Authorization Server가 아는 `client_id`를 마련했다.
이제 사용자를 Authorization Server로 보내 login하게 하고, 그 사용자를 대신할 access token을 받는다.
...
두 client가 어느 메시지에서 다르게 움직이는지는 [2장](02-why-oauth.md)의 표에 있다.
...
| `nonce` | ID token에 들어가 재사용을 막는 OpenID Connect 값이다. Spring Security가 `openid` 요청에 붙인다 |
...
`resource`는 RFC 8707 "Resource Indicators for OAuth 2.0"이 정한 parameter다.
client는 이 값으로 token을 어디에 쓸지 Authorization Server에 알린다.
값은 3장에서 읽은 PRM의 `resource`이고, `#` 뒤의 fragment가 없는 절대 주소다.
...
1. client는 무작위 문자열 `code_verifier`를 만들어 자기만 가지고 있는다.
...
| 막는 공격 | 방법 | PKCE가 막는 이유 |
|---|---|---|
| code 가로채기 | 공격자가 새어 나간 code를 자기가 token endpoint에 보낸다 | 공격자에게는 `code_verifier`가 없어서 `invalid_grant`로 거절된다 |
| code 주입 | 공격자가 자기 계정으로 받은 code를 피해자의 callback에 넣는다 | 피해자의 client가 보내는 `code_verifier`는 공격자의 code에 묶인 `code_challenge`와 맞지 않는다 |

요청을 시작한 쪽이 공격자라면 `code_verifier`도 공격자에게 있어서, PKCE로는 막지 못한다.
public client에서는 consent가 이 경우를 막는다(5.5).
official의 Authorization Server는 두 client 모두 `require-proof-key: true`여서, `code_challenge`가 없는 요청을 `invalid_request`로 거절한다.

## 5.5 Login과 consent

browser가 authorization request를 가져오면, Authorization Server는 먼저 요청을 검증한다.
...
public client는 누구나 그 `client_id`를 댈 수 있어서, Authorization Server는 요청을 보낸 프로그램이 진짜인지 확인할 수 없다.
그래서 public client에게는 매번 consent를 받아, 사용자가 자기가 시작하지 않은 요청을 알아채게 한다(4장).
...
`openid`만 있는 요청에 consent를 묻지 않는 것은, `openid`가 사용자 데이터를 읽는 권한이 아니라 사용자가 누구인지 묻는 요청이기 때문이다.
그러나 public client에서는 이 길로 consent 없이 code가 나간다.
다른 프로그램이 `scope=openid`로 요청하면 사용자 모르게 code를 받는다.
그 code로 받은 access token의 `aud`도 MCP Server라서, 그 프로그램은 사용자 이름으로 MCP Server를 부를 수 있다.
...
그래서 client는 code를 token endpoint로 보내기 전에, 자기가 보낸 요청의 응답인지(`state`)와 요청을 보낸 Authorization Server가 보냈는지(`iss`)를 확인한다.
...
`state`를 확인하지 않으면, 공격자는 자기 계정으로 받은 code를 붙인 callback 주소를 피해자의 browser가 열게 할 수 있다.
그러면 피해자는 모르는 사이에 공격자의 계정으로 login한다(CSRF).
...
callback을 통과한 code는 token endpoint에서 access token으로 바꾼다.
...
두 요청은 client를 증명하는 방법이 다르다.
`shop-agent`는 `Authorization: Basic` header에 `client_secret`을 넣는다.
`local-client`는 header 없이 본문에 `client_id`만 넣고, `code_verifier`가 비밀의 역할을 한다.
응답은 `refresh_token`이 없다는 점만 다르다(5.9).
...
|---|---|---|
| 쓰는 곳 | MCP Server. client가 MCP 요청마다 `Authorization: Bearer`로 붙인다 | client 자신. login한 사용자가 누구인지 알려 준다 |
| `aud` | `resource`로 받은 MCP Server 주소 | `client_id` |

두 token의 `sub`는 모두 login한 사용자(`user`)다.
MCP Server는 `aud`에 자기 이름이 있는 token만 받으므로, ID token을 보내면 `401`로 거절한다(6장).
`local-client`가 받은 access token의 `aud`도 `http://localhost:8111/mcp`다.
`aud`는 client 종류가 아니라 `resource`로 정해진다.
access token의 수명은 `exp`에서 `iat`를 뺀 300초이고, 응답의 `expires_in`은 `299`다.

Spring Authorization Server는 기본으로 access token의 `aud`에 `client_id`를 넣는다.
official에서는 `ResourceAudienceTokenCustomizer`가 access token의 `aud`만 `resource` 값으로 바꾸고, ID token은 그대로 둔다(5.11).
...
새 access token의 `aud`는 그대로 `http://localhost:8111/mcp`이고, token ID인 `jti`만 새 값이다.
...
agent는 MCP 요청을 보내기 전에, 만료됐거나 60초 안에 만료될 access token을 refresh한다.
그래서 만료된 token 때문에 `401`을 받고 다시 시도하는 일이 없다.

`local-client`의 token 응답에는 `refresh_token`이 없다.
Spring Authorization Server는 비밀 없이(`none`) 인증하는 client에게 refresh token을 주지 않는다(이유는 4장).
그래서 `local-client`는 token이 만료되면 authorization request부터 다시 한다.
...
`withPkce()`는 `code_verifier`를 session의 요청 기록에 두고, `code_challenge`와 `S256`을 주소에 넣는다.
`ResourceIndicators`는 discovery로 얻은 PRM의 `resource`를 요청에 넣는 도우미 클래스다.
...
`ResourceIndicatorValidator`는 `resource`가 없는 요청은 통과시킨다.
그렇게 받은 token은 5.7에서 본 대로 MCP Server에서 거절된다.
...
`ResourceAudienceTokenCustomizer`는 Spring Authorization Server가 JWT를 만들기 직전에 불린다.

```java
public void customize(JwtEncodingContext context) {
    if (!OAuth2TokenType.ACCESS_TOKEN.equals(context.getTokenType())) {
        return;                                                        // ID token의 aud는 client_id 그대로
    }
    Object requested = requestedResource(context);                     // token request의 resource
    Object authorized = authorizedResource(context);                   // authorization request의 resource
    ...
    /* authorization request에 없던 resource도 invalid_target */

    Object resource = (requested != null) ? requested : authorized;    // refresh에서 빠지면 처음 값
    /* 둘 다 없으면 aud를 바꾸지 않는다. 목록에 없는 resource는 invalid_target */
    context.getClaims().audience(List.of((String) resource));          // aud = MCP Server
}
```

`authorizedResource`는 저장된 authorization에서 처음 authorization request의 `resource`를 꺼내므로, refresh request에도 같은 규칙이 적용된다.
...
- MCP의 authorization은 PKCE를 쓰는 authorization code grant에 `resource`와 `iss` 확인을 더한 것이다.
...
| 내용 | 명세 | 요구 수준 |
|---|---|---|
| client는 PKCE를 구현하고, 가능하면 `S256`을 쓴다. 진행하기 전에 metadata로 PKCE 지원을 확인한다 | [MCP 2025-11-25 Authorization — Authorization Code Protection](https://modelcontextprotocol.io/specification/2025-11-25/basic/authorization#authorization-code-protection), [RFC 7636 §4.2](https://www.rfc-editor.org/rfc/rfc7636#section-4.2), [OAuth 2.1 §4.1.1](https://datatracker.ietf.org/doc/html/draft-ietf-oauth-v2-1-13#section-4.1.1) | MUST |
| `code_verifier`는 요청마다 새로 만든다. Authorization Server는 PKCE를 강제하고, `code_challenge`가 없으면 `invalid_request`, 맞지 않으면 `invalid_grant`로 답한다 | [RFC 7636 §4.1](https://www.rfc-editor.org/rfc/rfc7636#section-4.1), [§4.4.1](https://www.rfc-editor.org/rfc/rfc7636#section-4.4.1), [§4.6](https://www.rfc-editor.org/rfc/rfc7636#section-4.6), [OAuth 2.1 §7.5.2](https://datatracker.ietf.org/doc/html/draft-ietf-oauth-v2-1-13#section-7.5.2) | REQUIRED, MUST |
| client는 authorization request와 token request 모두에 MCP Server의 canonical URI를 `resource`로 넣고, Authorization Server가 지원하지 않아도 보낸다 | [MCP 2025-11-25 Authorization — Resource Parameter Implementation](https://modelcontextprotocol.io/specification/2025-11-25/basic/authorization#resource-parameter-implementation) | MUST |
| `resource`는 fragment 없는 절대 URI다. 받아들일 수 없는 값은 `invalid_target`이고, token request의 값은 원래 허가된 resource로 제한할 수 있다. access token은 특정 resource server로 대상을 제한한다 | [RFC 8707 §2](https://www.rfc-editor.org/rfc/rfc8707#section-2), [§2.1](https://www.rfc-editor.org/rfc/rfc8707#section-2.1), [§2.2](https://www.rfc-editor.org/rfc/rfc8707#section-2.2), [OAuth 2.1 §7.3.2](https://datatracker.ietf.org/doc/html/draft-ietf-oauth-v2-1-13#section-7.3.2) | MUST, MUST NOT, SHOULD |
...
| scope를 생략한 요청은 기본값으로 처리하거나 `invalid_scope`로 거절한다 | [RFC 6749 §3.3](https://www.rfc-editor.org/rfc/rfc6749#section-3.3), [§4.1.2.1](https://www.rfc-editor.org/rfc/rfc6749#section-4.1.2.1) | MUST |
...
| token request의 `code_verifier`는 `code_challenge`가 있었으면 필수이고, 인증하지 않는 client는 `client_id`를 보낸다. Authorization Server는 code 하나로 token을 한 번만 발급한다 | [OAuth 2.1 §4.1.3](https://datatracker.ietf.org/doc/html/draft-ietf-oauth-v2-1-13#section-4.1.3), [§3.2.4](https://datatracker.ietf.org/doc/html/draft-ietf-oauth-v2-1-13#section-3.2.4) | REQUIRED, MUST |
