...
1장에서 본 JSON-RPC 메시지에는 보낸 사람을 적는 field가 없다.
HTTP로 열린 MCP Server에는 주소를 아는 누구나 이 메시지를 보낼 수 있다.
...
1장에서 본 session ID는 이 질문에 답하지 못한다.
session ID는 연결을 가리킬 뿐, 보낸 사람을 증명하지 않는다.

가장 단순한 방법은 사용자가 MCP client에 password를 맡기고, client가 그 password로 MCP Server에 login하는 것이다.
그런데 MCP client와 MCP Server는 대개 만든 곳이 다르다.
Claude Desktop에 쇼핑몰 password를 넣으면, Claude Desktop은 그 password로 사용자의 계정에서 무엇이든 할 수 있다.
OAuth는 남이 만든 앱에 password를 넘기지 않고 권한만 빌려주는 방법이다.

OAuth에서 사용자는 Authorization Server에서 직접 login하고, client에게 무엇을 허락할지 정한다.
client는 password 대신 access token을 받는다.
access token에는 누구를 대신하는지, 어느 server에서 쓸 수 있는지, 언제까지 쓸 수 있는지가 들어 있다.
official practice에서 발급된 access token의 payload는 다음과 같다.
...
| claim | 뜻 |
|---|---|
| `iss` | 이 token을 발급한 Authorization Server |
| `sub` | client가 대신하는 사용자 |
| `aud` | 이 token을 받아 줄 server. 여기서는 MCP Server다 |
| `scope` | 사용자가 허락한 범위 |
| `exp` | 만료 시각. official에서는 발급하고 5분 뒤다 |

MCP Server는 token을 검증한 뒤 `sub`를 요청한 사용자로 본다.
official의 `shop-mcp-server`는 tool을 실행할 때마다 그 사용자를 로그에 남긴다.
...
MCP에서 authorization은 선택이다.
누구에게나 열어 둔 MCP Server도 있을 수 있다.
authorization을 쓰는 HTTP MCP Server는 OAuth 2.1을 바탕으로 한 MCP authorization 명세를 따른다.
1장에서 본 대로 stdio server는 이 명세 대신 환경 변수로 credentials를 받는다.

이 장은 누가 어떤 역할을 맡는지, 일반 OAuth와 무엇이 다른지, 전체 흐름이 어떻게 이어지는지를 한 번에 본다.
...
Authorization Server는 사용자를 login시키고, MCP Server에서 쓸 access token을 발급한다.
...
| Authorization Server | authorization server | 사용자를 login시키고 consent를 받아 token을 발급한다 | `auth-server`(`:9010`) |
...
비밀이 없으면 누군가 authorization code를 가로챘을 때 그 code를 막을 장치가 하나 줄어든다.
...
| token의 대상을 MCP Server로 좁힌다 | token을 쓸 API가 정해져 있어 대상을 밝히지 않는 경우가 많다 | client가 `resource`로 MCP Server를 밝히고, token의 `aud`에 그 MCP Server가 들어간다 | [5장](05-authorization-and-token.md) |
| public client가 흔하다 | 서버에서 도는 web 앱이 많다 | desktop 앱·명령줄 도구가 많다. 비밀이 없어서 PKCE가 authorization code를 지킨다 | [5장](05-authorization-and-token.md), [7장](07-local-client.md) |
| MCP 전송 자체의 보안 | token 검증이 API 보안의 중심이다 | 사용자 기기에서 도는 MCP Server도 있다. browser를 거친 공격을 막으려고 `Origin`·`Host`도 검사한다 | [6장](06-mcp-call-and-validation.md) |

세 번째와 다섯 번째 항목은 이유를 조금 더 보자.
...
MCP Server는 사용자 기기의 `localhost`에서 돌기도 한다.
사용자가 연 악성 web page는 DNS rebinding으로 browser를 거쳐 그 MCP Server에 요청을 보낼 수 있다.
official의 MCP Server는 token을 검사하기 전에 `Origin`과 `Host` header를 먼저 보고, 허용하지 않은 값이면 거절한다.

## 2.5 MCP Server와 Authorization Server의 분리

2025-03-26 버전의 MCP 명세에서 client는 MCP Server 주소에서 path를 뗀 곳에서 Authorization Server Metadata를 찾았다.
...
metadata가 없으면 같은 host의 `/authorize`, `/token`, `/register`를 기본 endpoint로 썼다.
그래서 Authorization Server의 metadata는 MCP Server와 같은 host에 있어야 했고, 명세의 예시도 MCP Server가 Authorization Server를 겸하는 구성이었다.

이 구성에서는 MCP Server를 만드는 쪽이 login, token 발급, client 등록까지 만들어야 했다.
회사가 이미 쓰는 Authorization Server가 다른 host에 있으면 MCP Server가 중간에 서야 했다.
MCP Server가 그 Authorization Server의 client가 되어 token을 받고, MCP client에게는 자기 token을 따로 발급하는 방식이다.

2025-06-18 버전은 MCP Server를 OAuth resource server로 정했다.
MCP Server는 PRM으로 자기를 보호하는 Authorization Server가 어디인지 알려 주기만 한다(3장).
같은 버전에서 client는 `resource`로 대상 MCP Server를 밝혀 token을 요청하게 됐다.

이제 MCP Server는 token을 검증만 하고, token 발급은 따로 있는 Authorization Server가 맡는다.
한 Authorization Server가 여러 MCP Server를 함께 맡을 수 있어서, 2.4에서 본 token 대상 좁히기가 같이 필요해졌다.
명세는 두 서버를 같은 곳에 두는 것도 막지 않는다.
official은 `auth-server`와 `shop-mcp-server`를 다른 process로 띄운다.
버전별 변화는 9장에서 다시 정리한다.
...
| 구간 | 메시지 | 하는 일 | 다루는 장 |
|---|---|---|---|
| client 등록 | 흐름 전 | Authorization Server가 아는 `client_id`를 미리 마련한다 | [4장](04-client-registration.md) |
...
| (9)(10) | login만 한다. consent 화면은 건너뛴다 | login한 뒤 consent 화면에서 `profile`을 고른다. 매번 consent를 받는다 |
...
## 2.7 각자 미리 아는 것과 알아내는 것

discovery가 있으면 무엇을 설정에 적고 무엇을 실행 중에 알아내는지 헷갈리기 쉽다.
신뢰의 출발점은 각자 설정에 미리 적어 둔 값뿐이다.
실행 중에 받은 값은 이 출발점과 맞춰 본 뒤에 쓴다.
...
| 사용자·browser | 없다 | Authorization Server와 agent의 session cookie만 가진다 |
...
`local-client`의 `Main#run`은 2.6의 흐름을 위에서 아래로 한 번 밟는다.
단계마다 클래스 하나가 맡는다.
...
`CLIENT_ID`는 `local-mcp-client`이다.
...
browser로 `http://localhost:8110`에 들어가면 `auth-server`의 login 화면으로 넘어간다.
`user`/`password`로 login하고 "p1 재고 알려줘"라고 묻는다.
`practice/mcp-security-authn-official/logs/shop-mcp-server.log`에 `사용자=user`가 적힌 tool 호출 줄이 남는다.

**local-client로 해 보기**

`run.sh`가 띄운 서버를 그대로 두고, 저장소 최상위 폴더에서 다음을 실행한다.
...
출력의 `[1]`~`[5]`가 2.6의 구간 순서와 같다.

```text
[1] discovery: http://localhost:8111/mcp
    Authorization Server: http://localhost:9010
[2] browser에서 login과 consent를 한다
    http://localhost:9010/oauth2/authorize?response_type=code&client_id=local-mcp-client&...
...
- 흐름은 PKCE를 쓰는 authorization code grant 그대로이고, MCP는 discovery, client 등록, `resource`·`aud`, public client 보호, 전송 보안을 더한다.

## 2.11 명세 근거

| 내용 | 명세 | 요구 수준 |
|---|---|---|
| authorization은 선택이다. HTTP transport는 이 명세를 따르고, stdio transport는 환경에서 credentials를 가져온다 | [MCP 2025-11-25 Authorization — Protocol Requirements](https://modelcontextprotocol.io/specification/2025-11-25/basic/authorization#protocol-requirements) | OPTIONAL, SHOULD, SHOULD NOT |
...
| 전체 흐름: `401` → PRM → metadata → 등록 → authorization request → callback → token request → MCP 요청 | [MCP 2025-11-25 Authorization — Authorization Flow Steps](https://modelcontextprotocol.io/specification/2025-11-25/basic/authorization#authorization-flow-steps) | — |
| 2025-03-26에는 metadata 주소를 MCP Server 주소에서 path를 뗀 base URL로 정했고, metadata가 없으면 `/authorize`·`/token`·`/register`를 썼다 | [MCP 2025-03-26 Authorization — Authorization Base URL](https://modelcontextprotocol.io/specification/2025-03-26/basic/authorization#authorization-base-url), [Fallbacks for Servers without Metadata Discovery](https://modelcontextprotocol.io/specification/2025-03-26/basic/authorization#fallbacks-for-servers-without-metadata-discovery) | MUST |
