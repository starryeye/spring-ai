...
`org.springaicommunity` 의 MCP 보안 module 3종(`0.1.14`) 위에, [`mcp-security-authn-official`](../mcp-security-authn-official)과 같은 MCP authorization 표준을 구현한다.
...
module 3종이 지원하지 않는 부분(Authorization Server Metadata discovery, PKCE 지원 확인, `resource` 싣기와 검증, `iss`)은 직접 쓴 클래스로 채운다.
재료(module 의 자동 구성과 확장점)만 official 과 다를 뿐 결과와 학습 결론은 같다.

## 구성

| 앱 | 역할 | 보안 module |
|---|---|---|
| `auth-server` | token 을 발급한다 | `mcp-authorization-server-spring-boot` |
| `shop-mcp-server` | token 을 검증한다. 없으면 `401` | `mcp-server-security-spring-boot` |
| `shop-agent` | token 을 얻어서 붙인다. browser UI + Ollama LLM | `mcp-client-security-spring-boot` |

포트·issuer·client_id·로그인 계정은 [허브의 포트·계정 표](../MCP-AUTHORIZATION.md#s2-ports) 한 곳에 있고, official 과 포트가 달라 두 practice 를 함께 띄울 수 있다.
세 앱 모두 Spring Boot 4.1.0 · servlet 스택이고, 앱 하나가 발급·검증·사용 중 하나씩만 담당한다.

## module 별 역할

각 module 은 `-spring-boot` 접미 변형을 쓴다. 접미 없는 core module(`mcp-server-security` 등)을 전이 의존으로 끌어오면서 자동 구성을 얹어 준다.
아래 "자동으로 생기는 것" 표의 근거는 각 module 의 자동 구성 소스(`*-0.1.14-sources.jar`)다.
...
자세히: [API-SPEC.md](API-SPEC.md#providers) · [등록](SEQUENCES.md#registration) · [token 발급](SEQUENCES.md#diff-authorization-server)
...
| `dcrRegisteredClientRepository` | DCR 이 켜졌을 때(기본값)만 생겨 `spring.security.oauth2.authorizationserver.client.*` 를 매핑한다. 이 practice 는 DCR 을 꺼, official 처럼 Boot 의 `registeredClientRepository` 가 같은 yml 을 매핑한다 |
| `ResourceIdentifierAudienceTokenCustomizer`(module 기본 token customizer) | access token 은 `openid` 가 승인되면 건너뛴다. ID token 은 `aud` 를 token request 의 `resource` 로 덮어쓴다 |
| `McpNoScopeClientConsentNotRequired` | scope 가 없거나 `openid` 하나뿐이면 consent 를 생략하는 판정. module 이 늘 건다 |

### `mcp-server-security-spring-boot` — token 검증

`shop-mcp-server` 는 이 module 로 OAuth 2.1 resource server 가 된다.
자동 구성(`McpServerSecurityAutoConfiguration`)은 `issuer-uri` 설정 하나로 filter chain 을 통째로 켜지만, audience 검증을 켜는 길이 없다. 그래서 `SecurityConfig` 가 같은 확장점(`McpServerOAuth2Configurer`)을 직접 적용한다.
...
| `OriginValidationFilter` | `allowedOrigins`/`allowedHosts` 설정 시 `addFilterAfter(CorsFilter)` 로 자동 등록. 인증 filter 보다 앞이다 |
| `AudienceValidationJwtDecoder` | `validateAudienceClaim(true)` 일 때 decoder 를 감싸, 요청 URL 로 계산한 기대 `aud` 를 검증 |

### `mcp-client-security-spring-boot` — token 획득·부착

`shop-agent` 가 MCP 를 호출할 때 `Authorization` 헤더를 붙여 준다.
`McpOAuth2ClientAutoConfiguration`·`HttpClientStreamableHttpTransportAutoConfiguration` 이 thread-local 의 인증을 읽어 token 을 구해 붙이는 부분을 자동으로 한다.
...
official 이 손으로 쓴 클래스마다, community 는 module 이 자동으로 해주거나(module 자동 구성) module 의 확장점에 직접 얹거나(직접 얹은 확장) 같은 클래스를 그대로 쓴다.
행은 [official 의 직접 쓴 코드 표](../mcp-security-authn-official/README.md#직접-쓴-코드)의 클래스를 모두 담는다. official 과의 비교는 이 표 한 곳에 둔다.
...
| `PublicClientConsentService` | `authorizationConsentService` bean 으로 등록 | 같은 클래스 |
| `UserConfig` | 그대로 재사용, 폼 로그인 filter chain 은 module 이 제공 | 같은 클래스 |
| public client 의 `none` 인증 방식 광고 | `McpAuthorizationStandardConfig` 의 `authorizationServerMetadataCustomizer`(같은 customizer 람다) | 직접 얹은 확장 |
| OIDC discovery(`.oidc(...)`) 켜기 | `OidcDiscoveryConfig`(`Customizer<McpAuthorizationServerConfigurer>` bean) | 직접 얹은 확장 |
| Dynamic Client Registration(DCR) 끔 | `spring.ai.mcp.authorizationserver.dynamic-client-registration.enabled: false` | module 자동 구성(설정으로 끔) |
...
| official 의 클래스 또는 설정 | community 의 설정·확장점 | 제공 |
|---|---|---|
| `SecurityConfig`(`oauth2Login`+`oauth2Client` filter chain, PKCE resolver·`iss` 검증 filter 배치, `csrf.spa()`) | 같은 배선. module 에는 `@ConditionalOnDefaultWebSecurity` 가 없어 직접 정의해도 자동 구성과 부딪히지 않는다 | 같은 클래스 |
| `DiscoveredClientRegistrationRepository` | 같은 이름, `OAuth2ClientProperties` 에서 자격증명만 가져오고 나머지는 discovery 결과로 채운다 | 같은 클래스 |
| `McpAuthorizationDiscovery` | 401 challenge·PRM 조회는 module 의 `McpMetadataDiscoveryService` 에 맡긴다. issuer binding·Authorization Server Metadata discovery·PKCE 지원 확인은 직접 한다 | 직접 얹은 확장 |
...
| `McpSecurityConfig`(discovery 저장소·authorized client 서비스·매니저·transport customizer 를 직접 등록) | discovery 저장소·token 요청 client·authorized client 매니저만 직접 등록. transport customizer 는 module 이 대신한다 | 직접 얹은 확장 |
| `OAuth2TokenAttachingRequestCustomizer` | `HttpClientStreamableHttpTransportAutoConfiguration` 의 `preRegisteredClientCustomizer`(`OAuth2AuthorizationCodeSyncHttpRequestCustomizer`) | module 자동 구성 |
| `SecurityMcpTransportContextProvider` | `McpOAuth2ClientAutoConfiguration` 의 `McpClientCustomizer<SyncSpec>`(`AuthenticationMcpTransportContextProvider`) | module 자동 구성 |
| `ChatController`(`ShopAgentApplication` 의 `Hooks.enableAutomaticContextPropagation()` 으로 reactor thread 에 `SecurityContext` 를 옮긴다) | `ChatController` 가 `.contextWrite(AuthenticationMcpTransportContextProvider.writeToReactorContext())` 를 부른다 | 직접 얹은 확장 |
...
### module customizer 는 ID token 의 `aud` 를 `resource` 로 덮어쓴다

module 의 `ResourceIdentifierAudienceTokenCustomizer` 는 `openid` 가 승인된 access token 만 건너뛰고, ID token 에는 `aud=resource` 를 씌운다.
그대로면 access token 의 `aud` 는 Spring `JwtGenerator` 기본값인 client_id 로 남고, ID token 의 `aud` 는 `resource` 가 된다.
`ResourceAudienceTokenCustomizer` 가 그 뒤에 돌아 access token 에 `aud=resource` 를 넣고 ID token 의 `aud` 를 client_id 로 되돌린다(테스트 `AuthorizationServerStandardTest#access_token_의_aud_는_resource_이고_id_token_은_client_id_다`).
...
그래서 `shop-mcp-server` 는 `auth-server` 없이도 뜬다.

### token 으로 보호된 MCP Server 는 부팅 시점에 handshake 를 못 한다

`McpClientAutoConfiguration` 은 bean 생성 시점에 `McpSyncClient.initialize()` 를 즉시 부르지만, 그 시점에는 요청 스레드도 로그인한 사용자도 없다.
`spring.ai.mcp.client.initialized: false` 로 handshake 를 첫 채팅 요청으로 미루면, 그 대가로 첫 질문이 느려진다.

### 스트리밍에는 `.contextWrite(...)` 가 필수다

`AuthenticationMcpTransportContextProvider` 는 thread-local 에서 인증을 읽는데 `ChatClient.stream()` 의 reactor 체인은 요청 스레드 밖에서 돈다.
...
| OAuth2 client 등록이 정확히 1개 | 0개·2개 이상이면 customizer 가 WARN 한 줄 남기고 no-op |
| `shop-mcp-server`·`auth-server` 에 `SecurityFilterChain` 직접 정의 | `@ConditionalOnDefaultWebSecurity` 가 꺼져 module 설정이 물러난다(`shop-agent` 는 이 조건이 없어 예외) |
| `ChatController` 의 `.contextWrite(...)` | token 이 안 붙는다(DEBUG 한 줄만) |

## 비목표

- scope 검사, tool 단위 authorization → 후속 `mcp-security-authz`
- 사용자 여러 명, 역할 분리, MCP session 을 사용자에 묶기(module `sessionBinding()` 은 켜지 않는다, [허브 5.8](../MCP-AUTHORIZATION.md#s5-8))
...
- 캡처: [2026-09-12-community.txt](../../docs/superpowers/captures/2026-09-12-community.txt) · [2026-09-25-community-public-client.txt](../../docs/superpowers/captures/2026-09-25-community-public-client.txt) · [2026-09-25-listen-addresses.txt](../../docs/superpowers/captures/2026-09-25-listen-addresses.txt)
- 기준 practice: [`mcp-security-authn-official`](../mcp-security-authn-official)
