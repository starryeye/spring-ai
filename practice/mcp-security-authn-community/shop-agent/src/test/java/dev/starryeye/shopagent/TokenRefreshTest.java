 ...
 *
 * <p>community 는 {@code DefaultOAuth2AuthorizedClientManager} 를 쓴다 — 모듈의
 * 요청 커스터마이저가 서블릿 요청·응답을 함께 넘기기 때문이다. 이 매니저는 인가된
 * 클라이언트를 요청 속성으로 넘겨야 하므로 가짜 서블릿 요청·응답이 필요하다.
 */
class TokenRefreshTest {
    ...
    void 만료된_토큰을_resource_를_실어_갱신한다() {
        ClientRegistration registration = ClientRegistration.withRegistrationId("authserver")
                .clientId("shop-agent")
                .clientSecret("shop-agent-secret")
        ...
        var authorizedClientService = new InMemoryOAuth2AuthorizedClientService(registrations);
        ...
        authorizedClientService.saveAuthorizedClient(new OAuth2AuthorizedClient(registration, "user", expired,
        ...
        var authorizedClients = new AuthenticatedPrincipalOAuth2AuthorizedClientRepository(authorizedClientService);
        var manager = McpSecurityConfig.buildAuthorizedClientManager(registrations, authorizedClients,
                refreshTokenClient);
